...
void harcodeClients(eClient clients[],int len, int howMany)
{
	...
	eClient auxList[]=
	{
			{1,"Maria","Rodríguez",12561754,0},
			{2,"Marcos","Castro",1600145489,0},
			{3,"Marcela","Vazquez",127154785,0},
			{4,"Marcia","Parodi",1800458456,0},
			{5,"Maximo","Escalar",2202125478,0},
			{6,"Pedro","Benitez",1330458723,0},
			{7,"Lucas","Martinez",1479458745,0},
			{8,"Lucia","Gimenez",2100100289,0},
			{9,"Oscar","Lugares",160013548,0},
			{10,"Martin","Gutierrez",141456874,1}
...
//--------------------------------------------------------------------------------------------------------------------------
void SubMenuPrintClientAndActiveLoans(eClient clients[], eLoan loans[],int lenClient,int lenLoan)//PRINT DETAIL OF ONE CLIENT ACTIVE LOANS 7 SUB MENU///////////////////////////
{
  int option;
  int ClientId;
  int index;
  	  	  ...
  	  	  do{
  	  		  ...
  	  		  utn_getNumber(&option,"\n-------------------------------------------"
	  			  	  	  	  		"\n1- Ver prestamos de cada cliente"
		  	  	  	  				"\n2- Salir "
		  	  	  	  				"\n-------------------------------------------\n\n",
									" * La opción ingresada no es válida * ",1,2,10);
  	  		 if(option==1)
  	  		  	  	  	  {
  	  		  	  	  		 utn_getNumber(&ClientId,"1- Ingrese ID del cliente : ","Error\n",100,150,3);
  	  		  	  	  		 index= searchClient(ClientId,clients,lenClient);

  	  		  	  	  		  printLoansByClientId(clients[index],loans,lenClient,lenLoan);
...
//--------------------------------------------------------------------------------------------------------------------------------
void printOneClientAndClosedLoans(eClient client,eLoan loans[],int lenClient,int lenLoan)//PRINT CLIENT INFORMATION AND CUMBER OF ACTIVES LOANS
{
	...
	int closeLoans;
			...
			if(client.isEmpty==1)
			{
				closeLoans=serchCloseLoansbyBlientId(loans,client.id,lenLoan);
